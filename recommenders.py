...
Original file is located at
    https://colab.research.google.com/drive/10MdJHQn5eJ0R4jYa_xNm2QiPCwDiXoJI
"""
...
class MainRecommender:
    ...
    def __init__(self, data: pd.DataFrame, weighting: bool = True):
        ...
        if weighting:
            self.user_item_matrix = bm25_weight(self.user_item_matrix.T, K1=125, B=0.6).T
            print('bm25_weight')
    ...
    def _prepare_matrix(data: pd.DataFrame):
        # df_price_item = data.groupby(['user_id', 'item_id'])[['cost']].sum().reset_index()
        # df_price_basket = data.groupby(['user_id'])[['cost']].sum()
        # df_share_item = pd.merge(df_price_item, df_price_basket, on="user_id", how='left')
        # df_share_item['share_price_item'] = df_share_item['cost_x'] / df_share_item['cost_y']
        # col = ['user_id', 'item_id', 'share_price_item']
        # df_merge = pd.merge(data, df_share_item[col], on=['user_id', 'item_id'], how='left')
        # user_item_matrix = pd.pivot_table(df_merge,
        #                                   index='user_id',
        #                                   columns='item_id',
        #                                   values='share_price_item',  # Можно пробовать другие варианты
        #                                   aggfunc='mean',
        #                                   fill_value=0
        #                                   )
        ...
        user_item_matrix = pd.pivot_table(data,
                                          index='user_id',
                                          columns='item_id',
                                          values='price',  # Можно пробовать другие варианты
                                          aggfunc='sum',
                                          fill_value=0
                                          )
